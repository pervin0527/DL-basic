...
def translate_sentence(model, src_sentence, src_vocab, trg_vocab, src_tokenizer, device, max_len=50):
    ...
    translated_sentence = [trg_vocab.itos[token] for token in translated_tokens if token not in [trg_vocab['<sos>'], trg_vocab['<eos>'], trg_vocab['<pad>']]]
...
def main():
    ...
    with torch.no_grad():
        for src, trg in tqdm(test_dataloader, desc='Test', leave=False):
            src_sentence = ' '.join([src_vocab.itos[token] for token in src[0].cpu().numpy() if token not in [pad_token, sos_token, eos_token]])
            trg_sentence = ' '.join([trg_vocab.itos[token] for token in trg[0].cpu().numpy() if token not in [pad_token, sos_token, eos_token]])
            model_output = translate_sentence(seq2seq, src_sentence, src_vocab, trg_vocab, src_tokenizer, device, max_len)
