...
class Attention(nn.Module):
    def __init__(self, hidden_dim):
        super().__init__()
        self.attn = nn.Linear(hidden_dim * 3, hidden_dim)  # hidden_dim * 2 (encoder output) + hidden_dim (hidden state)
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        seq_len = encoder_outputs.size(0)  # 시점의 수 seq_len
        h = hidden.repeat(seq_len, 1, 1).transpose(0, 1)  # (batch_size, seq_len, hidden_dim)
        
        encoder_outputs = encoder_outputs.permute(1, 0, 2)  # (batch_size, seq_len, hidden_dim * 2)
        
        combined = torch.cat([h, encoder_outputs], 2)  # (batch_size, seq_len, hidden_dim * 3)
        ...
        v = self.v.unsqueeze(0).expand(encoder_outputs.size(0), -1).unsqueeze(1)  # (batch_size, 1, hidden_dim)
...
class AttentionDecoder(nn.Module):
    ...
    def forward(self, input, last_hidden, encoder_outputs):
        ...
        output = self.out(torch.cat([output, context], 1))  # (batch_size, trg_vocab_size)
